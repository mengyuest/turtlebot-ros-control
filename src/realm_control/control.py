...
class RobotControl():
    ...
    def get_front_laser(self):
        time.sleep(1)
        return self.laser_msg.ranges[180]

    def get_laser_full(self):
        time.sleep(1)
        # print(len(self.laser_msg.ranges), self.laser_msg.angle_min,  self.laser_msg.angle_max, self.laser_msg.range_min, self.laser_msg.range_max)
    ...
    def get_odom(self):
        ...
        while tf_ok == 0 and not rospy.is_shutdown():
            try:
                self.tf_listener.waitForTransform('/base_footprint', '/odom', rospy.Time(), rospy.Duration(1.0))
                tf_ok = 1
...
if __name__ == '__main__':
    ...
    try:
        LIDAR_MAX=360
        while True:
            ls = robotcontrol_object.get_laser_full()
            # print(ls)
            front_d = ls[LIDAR_MAX//2]
            left_d = ls[-1]
            right_d = ls[0]
            hit_d = 0.6
            minimum_d = np.min(np.array(ls[LIDAR_MAX//4:-LIDAR_MAX//4]))
            # print(minimum_d)
            if minimum_d < hit_d:
                if np.mean(np.array(ls[:LIDAR_MAX//2])) < np.mean(np.array(ls[LIDAR_MAX//2:])):
                    robotcontrol_object.rotate(-45)
                else:                
                    robotcontrol_object.rotate(+45)
            else:
                robotcontrol_object.move_straight_time("forward", 1, 0.3)
